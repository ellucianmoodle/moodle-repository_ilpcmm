...
class datatelcmmhelper {
    ...
    /**
     * Gets all properties required to perform a service call to the CMM.
     *
     * @return object - service
     */
    public function get_service_properties() {
        $service = new stdClass();
        $service->web_services_url = trim(get_config('datatelcmm', 'webservicesurl'));
        $service->shared_secret = trim(get_config('datatelcmm', 'sharedsecret'));
        $service->domain_name = trim(get_config('datatelcmm', 'domainname'));
        if ($service->domain_name != '') {
            $service->domain_name = $service->domain_name . '\\';
        }
        $service->service_timeout = trim(get_config('datatelcmm', 'servicetimeout'));
        if ($service->service_timeout == '') {
            $service->service_timeout = 20;
        }
        $service->use_ssl = trim(get_config('datatelcmm', 'usessl'));
        $service->ca_authority = trim(get_config('datatelcmm', 'sslcaauthority'));
        $service->cert_path = trim(get_config('datatelcmm', 'certpath'));
        $service->site_collection_url = trim(get_config('datatelcmm', 'sitecollectionurl'));
     ...
     */
    public function convert_date_to_external($originaldate, $date_format) {
        $date = $originaldate;
        // Sample date: Modified=/Date(1315422686000)/.
        $date = (str_replace(')/', '', str_replace('/Date(', '', $originaldate))) / 1000;

        $date = userdate($date, $date_format);
        return $date;
    }
     ...
     */
    public function convert_date_to_internal($originaldate) {
     ...
     */
    public function date_format_display($string_to_format, $date_format) {
     ...
     */
    public function build_service_header($iv, $shared_secret, $web_services_url, $domain_name) {
     ...
     */
    public function get_timestamp_token($web_services_url) {
        ...
        if (($timestamp != '') && (is_numeric($timestamp))) {
            ...
            $error =  "Could not retrieve a valid timestamp token to access this course materials repository. ".
     ...
     */
    public function create_service_token($shared_secret, $domain_name, $web_services_url, $iv) {
        global $USER;
     ...
     *
     * @param documenttitle - title of document
     * @param collectionname - name of the collection the document belongs to
     * @param datemodified - the date the document was last modified - in JSON format
     * @param maxlen - the maximum length of the resulting string
     *
     * @return string - formatted_title
     */
    public function format_title($documenttitle, $collectionname, $datemodified, $maxlen = 200, $date_format) {
     ...
     */
    public function get_curl_defaults($service_url, $use_ssl = 1, $ca_authority = 1, $cert_path, $service_timeout) {
        $options = array(
     ...
     * @param $date_format - the date format being used - e.g. %Y/%m/%d
     *
     * @return array defaults
     */
    public function validate_search_input($keyword, $start_date, $end_date, $date_format) {
    ...
    /**
     * Takes a date string and normalizes it into YYYY-MM-DD based on the 
     ...
     */
    public function normalize_date($date_format, $date) {
        switch ($date_format) {
            case "%d/%m/%Y":
                // Strtotime considers a date as non-US when using dashes.

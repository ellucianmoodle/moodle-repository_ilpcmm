...
if ($CFG->version >= 2012061800) {
    require_once($CFG->dirroot."/repository/datatelcmm/lib23.php");
} else {
    require_once($CFG->dirroot."/repository/datatelcmm/lib20.php");
}

class repository_datatelcmm extends repository_datatelcmm_abs {
    ...
    public function search($search_text) {
        ...
        $ret  = array();
        $ret['nologin'] = true;
        $ret['list'] = $this->execute_search($this->search_keyword,  $this->search_modified_after, $this->search_modified_before,
                $this->search_personal_only);
        return $ret;
    }
     ...
     * /
    public function print_search($search_text) {
        return "";
    }
    */
    ...
    private function execute_search($keyword, $modifiedbefore = '', $modifiedafter = '', $personalonly = 'false') {
        ...
        $service_header = $this->cmmhelper->build_service_header($this->iv, $this->shared_secret, $this->web_services_url,
                $this->domain_name);
        ...
        $defaults = $this->cmmhelper->get_curl_defaults($this->get_materials_service_url, $this->use_ssl, $this->ca_authority,
                $this->cert_path, $this->service_timeout);
        ...
        if (is_array($materials)) {
            foreach ($materials as $item) {
                $shorttitle = $this->cmmhelper->format_title((string)$item['Title'], (string)$item['CollectionName'],
                        (string)$item['Modified'], 200, $this->date_format);
                $date = (str_replace(')/', '', str_replace('/Date(', '', $item['Modified']))) / 1000;
                $list[] = array(
    ...
    public function print_login($ajax = true) {
        ...
        $search_modified_after->label = $this->cmmhelper->date_format_display(get_string('search_modified_after',
                'repository_datatelcmm'), $this->date_format);
        ...
        $search_modified_before->label = $this->cmmhelper->date_format_display(get_string('search_modified_before',
                'repository_datatelcmm'), $this->date_format);
     ...
     */
    private function build_service_url($keyword,  $modifiedafter = '', $modifiedbefore = '', $personalonly = 'false') {
        ...
        */
        ...
        if ($modifiedbefore != '') {
            $modifiedbefore = $param_modifiedbefore = '&modifiedBefore=' .
                    $this->cmmhelper->normalize_date($this->date_format, $modifiedbefore);
        }

        $param_modifiedafter = '';
        if ($modifiedafter != '') {
            $modifiedafter = $param_modifiedafter = '&modifiedAfter=' .
                    $this->cmmhelper->normalize_date($this->date_format, $modifiedafter);
        }

        // This is the 'links only' version of the plug-in; only return items of type 'Links'.
        $param_doctype = '&Type=Files';
    ...
    public function get_file($filepath, $saveas = '') {
        ...
        $defaults = $this->cmmhelper->get_curl_defaults($filepath, $this->use_ssl, $this->ca_authority, $this->cert_path,
    ...
    /**
     * Admin settings page - Moodle 2.3+ version
     *
     * @param $mform - admin settings form
     */
    public static function type_config_form_23($mform) {

        $sitecollectionurl = get_config('repository_datatelcmm', 'sitecollectionurl');
        $webservicesurl = get_config('repository_datatelcmm', 'webservicesurl');
        $sharedsecret = get_config('repository_datatelcmm', 'sharedsecret');
        $domainname = get_config('repository_datatelcmm', 'domainname');
        $dateformat = get_config('repository_datatelcmm', 'dateformat');
        $maxresults = get_config('repository_datatelcmm', 'maxresults');
        $servicetimeout = get_config('repository_datatelcmm', 'servicetimeout');
        ...
        $mform->addElement('select', 'dateformat', get_string('dateformat', 'repository_datatelcmm'), $dateoptions);

        $mform->addElement('text', 'maxresults', get_string('maxresults', 'repository_datatelcmm'), array('size' => 2));
        $mform->setDefault('maxresults', 50);
        $mform->addRule('maxresults', 'Please specify a valid number', 'numeric', '', 'client');

        $mform->addElement('text', 'sitecollectionurl', get_string('sitecollectionurl', 'repository_datatelcmm'),
                array('maxlength' => 255, 'size' => 60));
        $mform->addRule('sitecollectionurl', $strrequired, 'required', null, 'client');

        $mform->addElement('text', 'webservicesurl', get_string('webservicesurl', 'repository_datatelcmm'),
                array('maxlength' => 255, 'size' => 60));
        $mform->addRule('webservicesurl', $strrequired, 'required', null, 'client');
        $mform->addElement('text', 'domainname', get_string('domainname', 'repository_datatelcmm'));
        $mform->addElement('html', get_string('domainexplanation', 'repository_datatelcmm'));

        $mform->addElement('text', 'servicetimeout', get_string('servicetimeout', 'repository_datatelcmm'), array('size' => 2));
        $mform->setDefault('servicetimeout', 20);
        $mform->addRule('servicetimeout', 'Please specify a valid number', 'numeric', '', 'client');

        $mform->addElement('passwordunmask', 'sharedsecret', get_string('sharedsecret', 'repository_datatelcmm'));
        $mform->addRule('sharedsecret', $strrequired, 'required', null, 'client');

        $mform->addElement('html', '<hr>');

        $mform->addElement('selectyesno', 'usessl', get_string('usessl', 'repository_datatelcmm'));

        $mform->addElement('selectyesno', 'sslcaauthority', get_string('sslcaauthority', 'repository_datatelcmm'));
        $mform->addElement('html', get_string('sslcawarning', 'repository_datatelcmm'));
        $mform->disabledIf('sslcaauthority', 'usessl', 'eq', 0);

        $mform->addElement('text', 'certpath', get_string('certpath', 'repository_datatelcmm'), array('maxlength' => 255,
                'size' => 60));
        $mform->addElement('html', get_string('certexplanation', 'repository_datatelcmm'));
        $mform->disabledIf('certpath', 'sslcaauthority', 'eq', 0);
    }


    /**
     * Admin settings page - Moodle 2.0, 2.1, 2.2 version
     *
     * @param $mform - admin settings form
     */
    public function type_config_form_20($mform) {

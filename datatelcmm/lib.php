...
class repository_datatelcmm extends repository {
   ...
   private function build_service_url($keyword,  $modifiedafter = '', $modifiedbefore = '', $personalonly = 'false') {
        ...
        */        
        ...
        if ($modifiedbefore != '') {            
            $modifiedbefore = strtotime($modifiedbefore, $this->time_format);                        
            $param_modifiedbefore = '&modifiedBefore=' . date('Ymd', $modifiedbefore);                            
        }
        
        $param_modifiedafter = '';        
        if ($modifiedafter != '') {
            $modifiedafter = strtotime($modifiedafter, $this->time_format);
    ...
    public function get_file($filepath, $saveas = '') {
        ...
        $defaults = $this->cmmhelper->get_curl_defaults($filepath, $this->use_ssl, $this->ca_authority, $this->cert_path, $this->service_timeout);                
        $options = array();        
        $ch = curl_init();        
        curl_setopt($ch,CURLOPT_HTTPHEADER,array('Authorization: '. $service_header));
        curl_setopt_array($ch, ($options + $defaults));
    ...
    */
    public function type_config_form($mform) {
        ...
        $mform->addElement('text', 'sitecollectionurl', get_string('sitecollectionurl', 'repository_datatelcmm'), array('maxlength' => 255, 'size' => 60));
        $mform->addRule('sitecollectionurl', $strrequired, 'required', null, 'client');
    
        $mform->addElement('text', 'webservicesurl', get_string('webservicesurl', 'repository_datatelcmm'), array('maxlength' => 255, 'size' => 60));
        ...
        $mform->addElement('text', 'certpath', get_string('certpath', 'repository_datatelcmm'), array('maxlength' => 255, 'size' => 60));            
